...
- [ ] `dist/_headers`, `dist/_redirects`, `dist/robots.txt`, `dist/sitemap.xml`, `dist/og-image.svg` 포함 확인
- [ ] 모바일 실기기 점검(선택): `npm run dev:mobile`
- [ ] 실제 도메인이 `https://isee.social/`이 아니면 `public/robots.txt`, `public/sitemap.xml`, `index.html`의 공유 메타 URL 수정
