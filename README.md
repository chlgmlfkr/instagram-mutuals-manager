...
Instagram Accounts Center에서 받은 내보내기 파일을 브라우저에서 분석해 `팔로우`, `팔로워`, `맞팔`, `언팔로워 후보`, `나를 팔로우함`, `제한`, `차단` 목록을 보여주는 정적 웹앱입니다.
...
차별점:
- 단순 `언팔로워 체크`가 아니라 `팔로워`, `팔로우`, `맞팔`, `언팔로워 후보`, `나를 팔로우함`, `차단`, `제한`을 한 번에 검토하는 관계 분석 워크스페이스로 포지셔닝
...
- 공개 배포 전 `ISeeSocial` 전용 로고를 만듭니다. 임시 `ISC` 마크는 유지하되, 최종적으로는 header mark, favicon, Open Graph 이미지까지 이어지는 `IS`/`ISC` 로고 시스템을 확정합니다.
...
- `팔로워`: 나를 팔로우 중인 계정
- `맞팔`: 서로 팔로우 중인 계정
- `언팔로워 후보`: 나는 상대를 팔로우하지만, 상대는 나를 팔로우하지 않는 상태
- `나를 팔로우함`: 상대는 나를 팔로우하지만, 나는 상대를 팔로우하지 않는 상태
...
- [ ] 실제 도메인이 `https://isee.social/`이 아니면 `public/robots.txt`, `public/sitemap.xml`, `index.html`, `public/privacy.html`, `public/instagram-export-guide.html`의 공유/링크 URL 수정
...
- UI 리모델링
  ...
  - 분석 완료 시 언팔로워 후보, 나를 팔로우함, 맞팔, 해석 실패를 최상단 요약에 노출
...
- 과거 리팩토링 단발 기록은 이후 `docs/project-log.md`와 README 작업 로그로 통합
- `README.md`의 폴더 구조 설명을 현재 배치에 맞게 갱신
- 문서성 파일은 `docs/`, 실행 스크립트는 `scripts/`, 샘플 데이터는 `ins/`에 두는 기준을 명시

### 2026-06-18
- 문서 기준을 `README.md`, `DESIGN.md`, `docs/project-log.md`, `docs/launch-checklist.md` 네 개로 정리
- 실행 완료된 UX 리모델링 프롬프트와 superpowers 계획 문서를 삭제
- ISeeSocial 공개 전 로고 시스템 작업을 디자인/런칭 체크리스트에 추가
